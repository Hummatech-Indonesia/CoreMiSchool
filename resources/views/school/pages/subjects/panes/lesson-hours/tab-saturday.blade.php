...
<div class="table-responsive rounded-2">
    <table class="table border text-nowrap customize-table mb-0 align-middle">
        ...
        <tbody>
            @forelse ($lessonHours['saturday'] ?? [] as $index => $lessonHour)
                <tr>
        ...
        <tfoot>
            <tr>
                <td>
                    <div>
                        <button type="button" data-id="{{ $latestHour[5][0] ? $latestHour[5][0]->id : '' }}" data-day="{{ $latestHour[5][0] ? $latestHour[5][0]->day : 'saturday' }}" data-name="{{ $latestHour[5][0] ? $latestHour[5][0]->name : '' }}" data-start="{{ $latestHour[5][0] ? $latestHour[5][0]->end : '' }}" class="btn-create btn btn-info btn-rounded m-t-10 mb-3">
                ...
                <td colspan="1"></td>
                @if (isset($lessonHours['saturday']))
                    <td>
                        <div class="mb-3">
                            <button data-id="{{ $latestHour[5][0] ? $latestHour[5][0]->id : '' }}" class="btn-delete btn btn-danger btn-sm">
